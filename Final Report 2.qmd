...
### The VEREP Parcel Dataset: Origins and Creation

Prior to this work, the [Center for Geospatial Solutions](https://www.lincolninst.edu/centers-initiatives/center-geospatial-solutions/) at the Lincoln Institute of Land Policy developed a comprehensive parcel dataset for Virginia Episcopal Real Estate Partners (VEREP) using an automated-plus-manual verification approach. The methodology combined CGS's [Who Owns America®](https://www.lincolninst.edu/centers-initiatives/center-geospatial-solutions/key-initiatives/) (WHOA) national parcel dataset with Diocese-specific knowledge to identify properties owned by the Episcopal Diocese of Virginia, its congregations, and related organizations.

#### CGS Methodology: Building the Foundation

To be able to view and query parcels owned by congregations in the Episcopal Diocese of Virginia, CGS utilized its WHOA parcel ownership dataset as a starting point. CGS created this dataset by cleaning, standardizing and combining parcel data from Regrid, additional property record data from ATTOM, and corporate entity registration data from OpenCorporates into a unified, national parcel dataset. 

Parcels are then assessed and classified, via an automated model created by CGS, into various ownership types. This includes a column to flag parcels, based on keyword searches of the parcel owner name field, that are likely affiliated with a faith-based institution.

#### Defining Ownership Parameters

The original Scope of Work defined the VEREP parcel dataset as a "dataset of parcels owned by congregations in the Episcopal Diocese of Virginia". However, following conversations with VEREP, the scope of ownership was expanded to broadly cover "parcels of interest to VEREP", where parcel ownership would fall into one of the following categories:

- Owned by the Episcopal Diocese of Virginia
- Owned by Congregations in the Episcopal Diocese of Virginia
- Owned by Related Organizations
- Owned by Affiliate Organizations

CGS relied on VEREP and its partners to define these categories and to provide starting lists of site addresses, congregation and organization names for CGS to review. CGS then performed a recursive parcel linkage process to identify all parcels connected through shared mailing addresses and owner names, maximizing the discovery of related property holdings across Virginia.

#### HDAdvisors's Role and the Congregation-Parcel Challenge

HDAdvisors was engaged after this comprehensive geospatial data creation and analysis work to further integrate congregation statistics.

CGS initially assumed a one-to-one relationship between parcels and congregations. However, field verification revealed that multiple congregations often share the same parcel (co-located congregations sharing worship space). This one-to-many relationship created a data structure challenge. 

The analysis performed in this report addresses this limitation by developing appropriate join methodologies to link congregation performance metrics (attendance, membership, pledge data) with parcel characteristics, enabling integrated spatial-statistical analysis of how congregation vitality relates to property characteristics and neighborhood context.

### Dataset Characteristics

The final integrated dataset includes:
...
ggplot(scoring_components, aes(x = reorder(Component, Weight), y = Weight)) +
  ...
  geom_text(aes(label = paste0(Weight, "%")), hjust = -0.2, size = 5, color = "#445ca9", fontface = "bold") +
  coord_flip() +
  ...
  theme(
    plot.title = element_text(family = "Roboto Slab", face = "bold", size = 16, color = "#445ca9"),
...
# Check if v_owntype column exists
if("v_owntype" %in% names(scored_parcels)) {
  ...
  # Create a simplified version based on available data
  ownership_stats <- scored_parcels %>%
    mutate(
      ownership_type = case_when(
        diocese_related == 1 ~ "Diocese-Related",
        has_congregation ~ "Congregation-Owned",
        TRUE ~ "Other"
      )
    ) %>%
    group_by(ownership_type) %>%
...
# Check if dio_region column exists, otherwise use diocese or create placeholder
if("dio_region" %in% names(scored_parcels)) {
...
} else if("diocese" %in% names(scored_parcels)) {
  # Use diocese column instead
  regional_stats <- scored_parcels %>%
    group_by(diocese) %>%
    summarise(
      n_parcels = n(),
  ...
  regional_stats %>%
    kable(
      col.names = c("Diocese/Region", "# Parcels", "Total Acres", "% of Total Acreage", "Avg Dev Score"),
      digits = c(0, 0, 1, 1, 1)
    ) %>%
    kable_styling(bootstrap_options = c("striped", "hover")) %>%
    row_spec(0, bold = TRUE, background = "#8baeaa", color = "white")
} else {
  # Show by county instead
  regional_stats <- scored_parcels %>%
...
```{r congregation-metrics, results='asis'}
...
cat("### All Developable Properties (Score ≥45)\n\n")
...
create_property_profile <- function(rank_num) {
  ...
  cat("- **Current Uses:**")
  uses <- c()
  # Add !is.na() checks to handle NA values
