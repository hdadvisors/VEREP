...
Prior to this work, the Lincoln Land Institute's Center for Geospatial Solutions (CGS) developed a comprehensive parcel dataset for Virginia Episcopal Real Estate Partners (VEREP) using an automated-plus-manual verification approach. The methodology combined CGS's Who Owns America® (WHOA) national parcel dataset with Diocese-specific knowledge to identify properties owned by the Episcopal Diocese of Virginia, its congregations, and related organizations.
...
CGS relied on VEREP and its partners to define these categories and to provide starting lists of site addresses, congregation and organization names for CGS to review.
...
"CGS was unable, in the project time allotment, develop a new relational table structure in Esri's web infrastructure to enable VEREP to query both congregation stats and parcel data in the same query."

The current analysis addresses this limitation by developing appropriate join methodologies to link congregation performance metrics (attendance, membership, pledge data) with parcel characteristics, enabling integrated spatial-statistical analysis of how congregation vitality relates to property characteristics and neighborhood context.
