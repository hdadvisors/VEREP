...
The development potential score is calculated by weighing each of the seven criteria according to its importance in determining development viability. Our scoring methodology, as outlined in the analytical framework of this report, reflects decades of real estate development experience and incorporates both market realities and mission-aligned priorities. Each criterion was selected to balance financial viability with community impact, ensuring that recommended properties can support sustainable development while serving congregational needs.

::: {.callout-tip}
This is just a starting point based on HDA's ideal of easy development for congregations. If VEREP identifies other criteria they would like to weigh differently, or has anecdotal information to add this scoring could be adjusted.
:::

### Property Size: Evaluating Development Potential at Any Scale

**Weight: 20%**

::: {.column-margin}
**Optimal Range**: 1.5-10 acres for standard development

Larger properties evaluated on merits, may require phasing.
:::

Properties of all sizes can present development opportunities, though the approach differs based on scale.
...
# Create scoring function - UPDATED: no penalty for large properties
calculate_size_score <- function(size) {
  case_when(
    size < 0.25 ~ 20,
    size < 0.5 ~ 40,
    size < 1.5 ~ 70,
    size < 10 ~ 100,    # Optimal range expanded
    size < 25 ~ 95,     # Large - still excellent
...
ggplot(size_curve, aes(x = acres, y = score)) +
  ...
  annotate("rect", xmin = 1.5, xmax = 10, ymin = 0, ymax = 100,
           alpha = 0.1, fill = "#8baeaa") +
  annotate("rect", xmin = 10, xmax = 30, ymin = 0, ymax = 100,
           alpha = 0.05, fill = "#445ca9") +
  annotate("text", x = 5.75, y = 50, label = "Optimal\nRange", 
           color = "#333", size = 4, fontface = "italic") +
  annotate("text", x = 20, y = 50, label = "Large/Campus\n(Phased Dev.)", 
           color = "#666", size = 3, fontface = "italic") +
  scale_x_continuous(breaks = seq(0, 30, 5)) +
  labs(
    title = "Property Size Scoring Curve (Updated)",
    subtitle = "Large properties no longer penalized",
...
property_profile |>
  filter(!is.na(area_acres), area_acres < 50) |>
  ggplot(aes(x = area_acres)) +
  geom_histogram(binwidth = 1, fill = "#445ca9", color = "white", alpha = 0.8) +
  geom_vline(xintercept = c(1.5, 10), linetype = "dashed", color = "#8baeaa", linewidth = 1) +
  annotate("text", x = 5.75, y = Inf, label = "Optimal Range", 
           vjust = 2, color = "#8baeaa", fontface = "bold") +
  labs(
    title = "Property Size Distribution",
    subtitle = "Dashed lines indicate optimal range (1.5-10 acres); larger properties also viable",
    x = "Property Size (Acres)",
...
- **Small (<0.5 acres)**: Limited development options, high per-unit costs
- **Standard (0.5-1.5 acres)**: Viable for smaller projects, townhomes, or ADUs
- **Optimal (1.5-10 acres)**: Maximum flexibility and economic efficiency
- **Large (10-25 acres)**: Excellent potential, may require phased approach
- **Campus Scale (25+ acres)**: Significant opportunity for master-planned development

Property size significantly influences development feasibility. Properties under half an acre typically lack the critical mass needed for financially viable development—construction costs per unit rise sharply, and parking or open space requirements become difficult to meet. 

**Updated Approach to Large Properties**: Previously, our methodology penalized parcels exceeding 5 acres due to them being unwieldy for smaller congregations. However, larger properties often represent the most significant development opportunities. While they may require phased development, patient capital, or more sophisticated planning, these sites can support transformative projects including:
...
The key consideration for large properties is not size itself, but whether environmental constraints, access, and market conditions support development.
...
**Indicators: Area Median Income, LIHTC Eligibility, and Household Income Growth Forecast**
...
Median income tells you where a market is today—but real estate development takes 3-5 years from concept to occupancy. Income growth forecast tells you where the market is heading, which is critical for timing development decisions
...
scoring_example <- tibble(
  Criterion = c("Land value", "Acreage", "Location", "Zoning", "Accessibility", "**Total**"),
  Weight = c(0.30, 0.25, 0.20, 0.15, 0.10, "—"),
  Score = c(80, 60, 90, 70, 50, "—"),
  Weighted = c(24, 15, 18, 10.5, 5, "**72.5**")
)

scoring_example |>
  kable(
    col.names = c("Criterion", "Weight", "Score", "Weighted Score"),
    align = c("l", "c", "c", "c")
  ) |>
  kable_styling(
...
These seven criteria create a holistic picture of development potential. A property might score exceptionally well on size and location but face significant challenges from environmental constraints. Another might have modest physical attributes but represent an urgent opportunity to support a financially stressed congregation.
...
Ultimately, successful church real estate development requires more than high scores. It demands patient capital, mission-aligned partners, engaged congregations, and creative design. But by systematically evaluating these criteria, we can identify the properties where stars may align—where market demand, congregational need, and regulatory environment likely converge to create genuine opportunity.
