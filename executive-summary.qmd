...
top_parcels |>
  ...
  column_spec(7, bold = TRUE,
              color = spec_color(head(top_parcels$development_score, 15), 
                                 scale_from = c(50, 80),
                                 palette = c("#e76f52", "#e9ab3f", "#8baeaa"),
                                 direction = 1))  
```
*Note: Properties without street addresses are identified by congregation name and location. These parcels have valid coordinates for mapping but lack specific addresses in county parcel records.*

```{r}
#| label: fig-tier-distribution
#| fig-cap: "Distribution of Parcels by Development Potential"
#| fig-height: 4
...
tier_summary <- scored_parcels |>
  ...
  mutate(
    pct = n / sum(n) * 100,
    # Set specific order to match chart
    development_potential = factor(development_potential, 
                                   levels = c("Constrained", "Too Large", "High", "Moderate", "Small Parcel"))
  )

ggplot(tier_summary, aes(x = development_potential, y = n, fill = development_potential)) +
  geom_col() +
  geom_text(aes(label = paste0(n, " (", round(pct, 1), "%)")), 
            vjust = -0.5, size = 3.5) +
  scale_fill_manual(values = c(
    "Constrained" = "#e9ab3f",      # Orange/yellow
    "Too Large" = "#e76f52",         # Dark gray
    "High" = "#445ca9",              # Blue
    "Moderate" = "#8baeaa",          # Teal/cyan
    "Small Parcel" = "#909090"       # Light gray
  )) +
...
This is further explained in the methodology [Chapter @sec-FinancialNeed]. 
