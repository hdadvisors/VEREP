...
**This analysis examines properties in the Episcopal Diocese of Virginia with fewer than 30 Sunday attendees, focusing on development potential through key metrics outlined in the analytical framework chapter.** This report was done for VEREP (Virginia Episcopal Real Estate Partners) in conjunction with case study projects HDAdvisors is facilitating.
...
Development tiers are based on composite scores that weigh physical characteristics, market conditions, and congregation financial health:
...
| Tier 5: Not Recommended | <30 | Unsuitable for development |

**Note:** Prior to exclusions, St Paul's on the Hill in Winchester was the only property to achieve Tier 1 status (score: 78+), validating the Diocese's selection of that site for affordable housing development. The remaining top opportunities fall within Tier 2, representing strong near-term candidates that may benefit from favorable market shifts or creative development partnerships.

For a detailed explanation of how properties are scored across each dimension, see the [Analytical Framework](analytical-framework.qmd) and [Development Criteria](development-criteria.qmd) sections.
:::

## At a Glance: Top 10 Parcelss
...
top_parcels |>
  slice_head(n = 10) |>
  select(rank, congregation_name, sadd, scity, scounty, rgisacre, 
         development_score, constraint_summary, development_tier) |>
  kable(
    col.names = c("Rank", "Congregation", "Address", "City", "County", 
                  "Acres", "Score", "Constraints", "Tier"),
    digits = c(0, 0, 0, 0, 0, 2, 1, 0, 0),
    align = c("c", "l", "l", "l", "l", "r", "r", "l", "l")
  ) |>
  kable_styling(
  ...
  column_spec(7, bold = TRUE,
              color = spec_color(
                head(top_parcels$development_score, 10) |> replace_na(50), 
                scale_from = c(50, 80),
                palette = c("#e76f52", "#e9ab3f", "#8baeaa"),
                direction = 1
              )) 
```
...
cat("Financial Need Congregation Parcels\n")
cat(":::\n\n")
...
This is further explained in the [methodology section](methodology.qmd#financial-need-aligning-development-with-mission).

### Geographic Distribution

```{r}
#| label: fig-geographic-dists
#| fig-cap: "Parcels by County"
#| fig-height: 4
...
ggplot(county_summary, aes(x = reorder(scounty, n_properties), y = n_properties)) +
  ...
  labs(
    title = "Counties with Most Parcels",
    x = "",
    y = "Number of Parcels"
  ) +
...
property_profile <- read_rds("data/output/property_profile_scored.rds")
