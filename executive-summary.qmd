...
top_parcels |>
  slice_head(n = 15) |>
  select(rank, congregation_name, sadd, scity, scounty, rgisacre, development_score, development_tier) |>
  kable(
    col.names = c("Rank", "Congregation", "Address", "City", "County", "Acres", "Score", "Tier"),
    digits = c(0, 0, 0, 0, 0, 2, 1, 0),
    align = c("c", "l", "l", "l", "l", "r", "r", "l")
  ) |>
  kable_styling(
...
ggplot(county_summary, aes(x = reorder(scounty, n_properties), y = n_properties)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  labs(
    title = "Top 10 Counties by Parcel Count",
    x = "",
    y = "Number of Parcelss"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold", color = "#445ca9"),
