...
This analysis utilizes the Virginia Episcopal Real Estate Portfolio (VEREP) parcel dataset, compiled by CGS Consultants. The dataset (originally 790 parcels) was further filtered to look at parcels/properties owned by congregations or the diocese and with small congregations (less than 30 regular members). This resulted in data encompassing:
...
This analysis also includes congregation statistics from the VEREP GIS webtool. Financial and membership data provides context on congregation health, including:
...
In the interim, we attempted to use other data present to fill in these gaps. This process is outlined below.

### The QCT Data Challenge

Qualified Census Tract (QCT) designation was originally included because it opens access to Low-Income Housing Tax Credits and other development incentives. However, zero properties in this portfolio qualified as QCTs, rendering this metric inapplicable.

Since the QCT pathway to financial viability was unavailable, we replaced it with median income and five-year household income growth to assess market-based viability instead. Properties in high-income, high-growth areas (scoring 90-95 points) demonstrate strong market fundamentals—resident purchasing power, economic stability, and growth potential—that support financial sustainability without relying on low-income housing incentives. Properties in declining markets score lower (35-40 points).

This approach shifts from measuring eligibility for government funding programs to assessing general market strength, using actual economic conditions at each property location rather than federal designations that proved inapplicable to small, rural congregations.
...
Rather than impute wetland data or exclude these properties from analysis, we implemented a **multi-faceted constraint assessment framework** using complementary data sources with comprehensive coverage to determine constraints. This includes:

**Historic District Designation**

We incorporated National Register Historic District boundaries from the Virginia Department of Historic Resources. This revealed that **45% of the 104 properties** are located within historic districts—a finding with significant implications for development feasibility.
