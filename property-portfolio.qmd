...
cat("## ", high_potential, "\n", sep = "")
cat("Strong Potential Properties\n")
cat(":::\n\n")
...
::: {.callout-tip}
## Methodology Validation: St Paul's on the Hill
In our initial analysis before exclusions, St Paul's on the Hill in Winchester received the highest development score in the portfolio. This independently validates the Diocese's decision to select this site for affordable housing development. The property has been excluded from current rankings as it is already in active development.
:::
...
tier_summary <- scored_parcels |>
  mutate(
    ...
    development_category = case_when(
      development_potential == "Low Potential - Small Parcel" ~ "Size-Restricted",
      development_potential == "High Potential" ~ "Strong",
      development_potential == "Moderate Potential" ~ "Moderate",
      development_potential == "Low Potential - Constraints" ~ "Constrained",
      TRUE ~ "Other"
    )
  ) |>
  count(development_category) |>
  filter(development_category != "Other") |>  # Remove any uncategorized
  mutate(
    pct = n / sum(n) * 100,
    label = paste0(n, " (", round(pct, 0), "%)"),
    potential_order = factor(development_category, 
                             levels = c("Strong", "Moderate", "Size-Restricted", "Constrained"))
  ) |>
...
scored_parcels |>
  mutate(
    size_category = case_when(
      area_acres < 0.5 ~ "< 0.5 acres (Small)",
      area_acres < 1.5 ~ "0.5–1.5 acres (Moderate)",
      area_acres <= 5 ~ "1.5–5 acres (Optimal)",
      area_acres <= 10 ~ "5–10 acres (Large)",
      TRUE ~ "10+ acres (Campus Scale)"
    ),
    size_category = factor(size_category, 
                           levels = c("< 0.5 acres (Small)", 
                                      "0.5–1.5 acres (Moderate)", 
                                      "1.5–5 acres (Optimal)", 
                                      "5–10 acres (Large)", 
                                      "10+ acres (Campus Scale)")),
    # Map actual development_potential values to display categories
    development_category = case_when(
      development_potential == "High Potential" ~ "Strong",
      development_potential == "Moderate Potential" ~ "Moderate",
      development_potential == "Low Potential - Small Parcel" ~ "Size-Restricted",
    ...
    development_category = factor(development_category,
                                   levels = c("Strong", "Moderate", "Size-Restricted", "Constrained", "Other"))
  ) |>
  filter(development_category != "Other") |>  # Remove uncategorized if any
...
The most high and moderate-potential properties concentrate in the 0.5–5 acre "sweet spot," while parcels under 0.5 acre face inherent limitations and the largest sites show more constraints. 11 properties (11%) are large sites (>10 acres) that may benefit from phased development approaches
...
if(nrow(high_potential_props) > 0) {
  high_potential_props |>
    ...
    select(Rank, sadd, scity, scounty, rgisacre, development_score, development_tier) |>
    kable(
  ...
  cat("No properties currently meet the High Potential development criteria.")
}
```
