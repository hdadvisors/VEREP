...
The following section provides detailed profiles for each of the top 10 development opportunity parcels (across 7 congregations). Each profile includes:
...
top_10_parcels <- property_profile |>
  ...
  slice_head(n = 10) |>
  mutate(parcel_rank = row_number())

high_potential_congregations <- top_10_parcels |>
  group_by(congregation_name) |>
  summarise(
    site_city = first(scity),
    site_county = first(scounty),
    n_parcels = n(),
    ...
    top_parcel_rank = min(parcel_rank),  # highest ranked parcel
    .groups = "drop"
  ) |>
  arrange(top_parcel_rank) |>
...
create_congregation_profile <- function(row_num) {
  ...
  cat("::: {.info-box}\n")
  cat("**⛪ Congregation:** ", cong$congregation_name, "\n\n", sep = "")
  if(!is.na(cong$avg_members)) {
    cat("**👥 Members:** ", round(cong$avg_members, 0), "\n\n", sep = "")
  }
  if(!is.na(cong$avg_pledge)) {
    cat("**💰 Avg Pledge:** ", scales::dollar(cong$avg_pledge, accuracy = 1), "\n", sep = "")
  }
  cat(":::\n\n")
  ...
  cat("::: {.info-box}\n")
  cat("**🎯 Why Engage:** ", engagement_rationale, "\n\n", sep = "")
  cat("**✅ Recommended Action:** ", recommended_action, "\n")
  cat(":::\n\n")
