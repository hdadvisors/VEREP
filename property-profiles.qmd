...
# First, create the congregation-level summary with priority ranking
priority_congregations <- property_profile %>%
  ...
  mutate(
    engagement_priority = case_when(
      congregation_name == "St Pauls on the Hill Church" ~ 1,
      congregation_name == "Good Shepherd-of-the-Hills" ~ 2,
      congregation_name == "Aquia Church" ~ 3,
      site_county %in% c("FAIRFAX", "FAIRFAX-CITY") ~ 4,
      congregation_name == "Grace Church Emmanuel Parish" ~ 5,
      TRUE ~ 6
    ),
    engagement_rationale = case_when(
      avg_attendance == 0 & mean_dev_score >= 70 ~ "Zero attendance with flagship development potential",
      avg_attendance == 0 ~ "Zero attendance suggests functional inactivity",
      avg_members > 500 & avg_attendance == 0 ~ "Data anomaly: high membership but zero attendance requires clarification",
      site_county %in% c("FAIRFAX", "FAIRFAX-CITY") ~ "Fairfax cluster merger potential",
      n_parcels >= 3 ~ "Multi-parcel consolidation opportunity",
      TRUE ~ "Standard development opportunity"
    ),
    recommended_action = case_when(
      avg_attendance == 0 ~ "Assess congregational status and readiness for transition",
      avg_attendance < 20 & n_parcels > 1 ~ "Explore parcel consolidation while maintaining worship presence",
      site_county %in% c("FAIRFAX", "FAIRFAX-CITY") ~ "Evaluate merger with nearby Fairfax congregations",
      TRUE ~ "Engage leadership on development partnership options"
    )
  ) %>%
  arrange(desc(mean_dev_score))  # Sort by development score

# Function to create congregation profile with tabsets
create_congregation_profile <- function(row_num) {
  cong <- priority_congregations[row_num, ]
  ...
  # Section header
  cat("\n\n## ", cong$congregation_name, " {.unnumbered}\n\n", sep = "")
  cat("**📍 ", cong$site_city, ", ", cong$site_county, " County**\n\n", sep = "")
  
  # Badges
  cat("[Dev Score: ", round(cong$mean_dev_score, 1), "]{.score-badge} ", sep = "")
  cat("[Rank: ", row_num, " of ", nrow(priority_congregations), "]{.score-badge}\n\n", sep = "")
  ...
  cat("::: {.metric-card}\n")
  cat("[", cong$n_parcels, "]{.metric-value}\n\n", sep = "")
  cat("[Parcels]{.metric-label}\n")
  cat(":::\n\n")
  
  cat("::: {.metric-card .teal}\n")
  cat("[", round(cong$total_acres, 1), "]{.metric-value .teal}\n\n", sep = "")
  cat("[Acres]{.metric-label}\n")
  cat(":::\n\n")
  
  cat("::: {.metric-card .", att_class, "}\n", sep = "")
  cat("[", round(cong$avg_attendance, 0), "]{.metric-value .", att_class, "}\n\n", sep = "")
  cat("[Avg Attendance]{.metric-label}\n")
  cat(":::\n\n")
  
  cat("::: {.metric-card .", walk_class, "}\n", sep = "")
  cat("[", round(cong$walkability_score, 1), "/20]{.metric-value .", walk_class, "}\n\n", sep = "")
  ...
  cat("::: {.info-box}\n")
  cat("**💰 Assessed Value:** ", scales::dollar(cong$total_value, accuracy = 1), "\n\n", sep = "")
  cat("**🎯 Why This Site:** ", cong$engagement_rationale, "\n")
  cat(":::\n\n")
  
  # Walkability callout if score is notable
  if(!is.na(cong$walkability_score) && cong$walkability_score >= 15) {
    cat("::: {.callout-tip}\n")
    cat("## 🚶 Highly Walkable Location\n\n")
  ...
  if(cong$avg_attendance == 0) {
    cat("::: {.status-box .warning}\n")
    cat("**⚠️ Zero Average Attendance**\n\n")
  ...
  cat("| Walkability Score | ", round(cong$walkability_score, 1), "/20 |\n", sep = "")
  ...
  if(cong$n_parcels > 1) {
    cat("::: {.callout-note}\n")
    cat("This congregation owns multiple parcels, which may present consolidation opportunities.\n")
    cat(":::\n\n")
  }
  ...
  cat("::: {.info-box}\n")
  cat("**🎯 Why Engage:** ", cong$engagement_rationale, "\n\n", sep = "")
  cat("**✅ Recommended Action:** ", cong$recommended_action, "\n")
  cat(":::\n\n")
  
  cat("#### Suggested Outreach Steps\n\n")
  ...
  } else if(cong$site_county %in% c("FAIRFAX", "FAIRFAX-CITY")) {
    cat("1. Identify nearby Episcopal congregations for merger discussions\n")
    cat("2. Facilitate joint vestry conversations about shared future\n")
    cat("3. Conduct combined property portfolio analysis\n")
    cat("4. Develop merger proposal with property optimization strategy\n")
...
for(i in 1:nrow(priority_congregations)) {
