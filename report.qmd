...
property_profile <- property_profile %>%
  mutate(
    # Add area_acres alias
    area_acres = rgisacre,
    
    # Add development_score based on potential
    ...
    # Add development_tier alias
    development_tier = development_potential,
    
    # Add assessed_value alias
    assessed_value = lan_val,
    
    # Add use flags (handle NA values)
    ...
    # Add placeholder fields for sections that need them
    avg_attendance = attendance_2023,
    avg_pledge = plate_pledge_2023,
    bldgcount = 1,               # Assume 1 building
    year_built = 1950,           # Default year
...
# Summary counts for narrative
cat("=== LOADED PROCESSED DATA ===\n")
cat(sprintf("Total properties analyzed: %d\n", nrow(property_profile)))
cat(sprintf("High/Moderate potential: %d\n", nrow(leadership_report)))
cat(sprintf("Need data collection: %d\n", nrow(data_followup)))
cat(sprintf("Top 10 properties identified: %d\n", nrow(top_10)))
```
...
This analysis evaluated **`r nrow(property_profile)`** properties across the diocese to identify optimal development opportunities. We identified **`r nrow(top_10)` high-priority properties** representing **`r round(sum(top_10$area_acres, na.rm = TRUE), 1)` total acres** with significant development potential.

**Top Insights:**
- **`r sum(property_profile$development_score >= 60, na.rm = TRUE)`** properties score in Tier 1-2 (strong development potential)
- **Primary opportunities:** Underutilized parking lots and open space
- **Average parcel size:** `r round(mean(property_profile$area_acres, na.rm = TRUE), 2)` acres
...
::: {.stat-box}
::: {.stat-number}
`r round(sum(top_10$area_acres, na.rm = TRUE), 1)`
:::
::: {.stat-label}
Developable Acres
:::
...
**`r sum(property_profile$development_score >= 60, na.rm = TRUE)` properties** score in Tier 1-2 (strong development potential), representing substantial untapped value across the diocese. Primary opportunities exist in:
...
- **`r nrow(property_profile)` parcels** across Virginia
...
One of the challenges revealed through CGS' original analysis was that many congregations share properties or do not accurately record their permanent address. As a result, some of the data used in this analysis had these persistent problems, which we attempted to sort and organize for future manual review. A large portion of the 126 parcels had incomplete data, which impacted the overall analysis. 

```{r summary-stats, echo=FALSE}

total_props <- nrow(property_profile)
incomplete_data <- sum(property_profile$data_completeness < 6)
missing_addresses <- sum(is.na(property_profile$sadd) | 
                        property_profile$sadd == "" | 
                        grepl("UNASSIGNED", property_profile$sadd, ignore.case = TRUE))
```

**Key Findings:**

- Total properties analyzed: `r total_props`
- Properties with incomplete data: `r incomplete_data/total_props*100` 
- Properties with missing/unassigned addresses: `r missing_addresses`

### Missing and Unassigned Addresses

These congregations have properties without proper street addresses, making them difficult to verify or inspect for development.  

```{r unassigned-analysis}
#| echo: false
...
which can be verified with county tax records. 



### Shared Addresses 
These properties have different congregations sharing the same street address.
...
shared_summary %>%
  gt() %>%
  tab_header(
    title = "Addresses Shared by Multiple Congregations",
    subtitle = "May indicate shared facilities or data errors"
  ) %>%
  cols_label(
    sadd = "Street Address",
    scity = "City",
    num_congregations = "# Congregations",
    congregations = "Congregation Names",
    total_attendance = "Combined Attendance"
  ) %>%
  tab_style(
    style = cell_fill(color = "#d1ecf1"),
    locations = cells_body(
      columns = num_congregations,
      rows = num_congregations >= 3
    )
  ) %>%
  fmt_number(
    columns = total_attendance,
    decimals = 0
  )
```

These properties will require verification with diocese or congregation before full determinations can be made.

**`r sum(full_comparison$has_unassigned & !full_comparison$has_shared_address)` congregations** with unassigned addresses - verify using coordinates or tax records.

**`r sum(full_comparison$has_shared_address & !full_comparison$has_unassigned)` congregations** with shared addresses - confirm ownership and arrangements.


### Other data quality issues

In addition to the challenges listed above, related to address matching, some properties were missing property and environmental data necessary for a full analysis. 
...
::: {.stat-box}
::: {.stat-number}
`r developable_summary$n_developable`
:::
::: {.stat-label}
Developable Parcels
:::
...
::: {.stat-box}
::: {.stat-number}
`r round(developable_summary$total_dev_acres, 1)`
:::
::: {.stat-label}
Developable Acres
:::
...
::: {.stat-box}
::: {.stat-number}
$`r scales::comma(developable_summary$total_dev_value / 1e6, accuracy = 0.1)`M
:::
::: {.stat-label}
Assessed Value
:::
...
::: {.stat-box}
::: {.stat-number}
`r developable_summary$parcels_under_30`
:::
::: {.stat-label}
With <30 Attendance
:::
