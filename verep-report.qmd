...
# Add geocoding if coordinates don't exist
if (!"longitude" %in% names(top_10) || !"latitude" %in% names(top_10)) {
  library(tidygeocoder)
...
create_property_profile <- function(rank_num) {
  ...
  # ADD INTERACTIVE ADU PLACEMENT HERE
  cat("\n#### Interactive ADU Placement Tool\n\n")
  cat("Explore potential ADU configurations on this property. Click on the map to place different ADU designs.\n\n")
  
  # Output the interactive UI using raw HTML
  cat('<div style="margin: 20px 0;">\n')
  cat('<div style="display: grid; grid-template-columns: 300px 1fr; gap: 20px;">\n')
  
  # Sidebar
  cat('<div style="background: #f8f9fa; padding: 15px; border-radius: 8px; height: fit-content;">\n')
  cat('<h5 style="margin-top: 0;">ADU Design Tool</h5>\n')
  cat('<div id="', prop_id, '_controls"></div>\n', sep = "")
  cat('<hr>\n')
  cat('<button id="', prop_id, '_clear" class="btn btn-warning btn-sm" style="width: 100%; margin-bottom: 10px;">Clear ADUs</button>\n', sep = "")
  cat('<button id="', prop_id, '_reset" class="btn btn-secondary btn-sm" style="width: 100%;">Reset View</button>\n', sep = "")
  cat('<hr>\n')
  cat('<div style="font-size: 0.85em; color: #666;">\n')
  cat('<strong>How to use:</strong>\n')
  cat('<ol style="margin: 10px 0; padding-left: 20px;">\n')
  cat('<li>Select an ADU design</li>\n')
  cat('<li>Click on the map to place</li>\n')
  cat('<li>Toggle between views</li>\n')
  cat('</ol>\n')
  cat('</div>\n')
  cat('</div>\n')
  
  # Map
  cat('<div id="', prop_id, '_map" style="height: 600px; border-radius: 8px; overflow: hidden;"></div>\n', sep = "")
...
for (i in 1:10) {
  local({
    idx <- i
    property <- top_10[idx, ]
    prop_id <- paste0("prop_", idx)
    
    # Skip if no coordinates
    if (is.na(property$longitude) || is.na(property$latitude)) {
      return(NULL)
    }
    
    # Reactive values
    placed_adus <- reactiveValues(adus = list(), counter = 0)
    
    parcel_lng <- property$longitude
    parcel_lat <- property$latitude
    
    # Create UI namespace
    ns <- NS(prop_id)
    
    # Render controls in the sidebar
    output[[paste0(prop_id, "_controls")]] <- renderUI({
      tagList(
        selectInput(
          ns("adu_type"),
          "Select ADU Design:",
        ...
        uiOutput(ns("adu_info"))
      )
    })
    
    # ADU info card
    output[[paste0(prop_id, "_adu_info")]] <- renderUI({
      req(input[[paste0(prop_id, "_adu_type")]])
      design <- adu_designs %>% filter(name == input[[paste0(prop_id, "_adu_type")]])
      
      div(
        style = sprintf(
          "background: %s; color: white; padding: 12px; border-radius: 5px; margin-top: 10px;",
          design$color
        ),
        tags$strong(design$name),
        tags$br(),
        tags$small(paste(design$sqft, "sqft |", design$width_ft, "' ×", design$length_ft, "'")),
        tags$br(),
        tags$small(paste(design$bedrooms, "bedroom(s)"))
      )
    })
    
    # Initialize map
    output[[paste0(prop_id, "_map")]] <- renderLeaflet({
      leaflet() %>%
        ...
        setView(lng = parcel_lng, lat = parcel_lat, zoom = 19) %>%
        addMarkers(
          lng = parcel_lng,
          lat = parcel_lat,
          label = property$congregation_name,
          popup = paste0(
            "<strong>", property$site_address, "</strong><br>",
            "Land Value: $", format(property$assessed_value, big.mark = ","), "<br>",
            "Acreage: ", round(property$area_acres, 2)
          )
        ) %>%
    ...
    # Place ADU on click
    observeEvent(input[[paste0(prop_id, "_map_click")]], {
      click <- input[[paste0(prop_id, "_map_click")]]
      req(input[[paste0(prop_id, "_adu_type")]])
      ...
      placed_adus$counter <- placed_adus$counter + 1
      adu_id <- paste0("adu_", prop_id, "_", placed_adus$counter)
      
      leafletProxy(paste0(prop_id, "_map")) %>%
        addPolygons(
          data = adu_geom,
          layerId = adu_id,
          ...
          popup = paste0(
            ...
            design$sqft, " sqft | ", design$bedrooms, " bed"
          ),
          group = "ADUs"
        )
      
      placed_adus$adus[[adu_id]] <- list(
        geom = adu_geom,
        design = design$name
      )
    })
    ...
    # Clear button
    observeEvent(input[[paste0(prop_id, "_clear")]], {
      leafletProxy(paste0(prop_id, "_map")) %>%
        clearGroup("ADUs")
      placed_adus$adus <- list()
      placed_adus$counter <- 0
    })
    
    # Reset view button
    observeEvent(input[[paste0(prop_id, "_reset")]], {
      leafletProxy(paste0(prop_id, "_map")) %>%
        setView(lng = parcel_lng, lat = parcel_lat, zoom = 19)
    })
  })
